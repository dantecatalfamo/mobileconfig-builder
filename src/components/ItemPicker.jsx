...
export function ItemPicker({ schemas, onAdd, label }) {
  ...
  const handleKeyDown = e => {
    ...
    else if (e.key === 'Enter' && filtered[activeIdx]) { handlePick(filtered[activeIdx]) }
    ...
    <div className="payload-picker">
      ...
      {open && (
        <div className="picker-panel">
          <input autoFocus type="text" placeholder={`Search ${label.toLowerCase()}…`} value={search}
            onChange={e => setSearch(e.target.value)} onKeyDown={handleKeyDown} className="picker-search" />
          <div className="picker-list" ref={listRef}>
            {filtered.map((id, i) => (
              <button key={id} className={`picker-item ${i === activeIdx ? 'active' : ''}`}
                onMouseEnter={() => setActiveIdx(i)} onClick={() => handlePick(id)}>
