  ...
  return (
    <span className="os-badges">
      {badges.map(({ os, version }) => (
        <span key={os} className="os-badge">
          {OS_SHORT[os] ?? os} {version}+
        </span>
      ))}
    </span>
  );
...
function DeprecationBadge({ supportedOS }) {
  ...
  const removed = entries
    .filter(([, i]) => i?.removed)
    .map(([os, i]) => `${OS_SHORT[os] ?? os} ${i.removed}`);
  const deprecated = entries
    .filter(([, i]) => i?.deprecated && !i?.removed)
    .map(([os, i]) => `${OS_SHORT[os] ?? os} ${i.deprecated}`);
  return (
    <>
      {removed.length > 0 && (
        <span className="key-removed">removed {removed.join(", ")}</span>
      )}
      {deprecated.length > 0 && (
        <span className="key-deprecated">
...
function DefaultBadge({ defaultVal }) {
  ...
  return (
    <span className="key-default" title={`Default: ${label}`}>
      default: {label.length > 24 ? label.slice(0, 24) + "…" : label}
    </span>
  );
}
...
function NotSupportedNote({ supportedOS, payloadSupportedOS }) {
  ...
  return (
    <span
      className="key-not-supported"
      title={`Not supported on: ${notOn.join(", ")}`}
    >
      not on {notOn.join(", ")}
    </span>
  );
...
export function FieldLabel({
  ...
  return (
    <div className="field-label">
      ...
      {description && (
        <>
          <button className="help-btn" onClick={() => setShow(s => !s)}>
            ?
          </button>
