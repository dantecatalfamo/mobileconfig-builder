...
export function DeclarativeMode({ schemasData }) {
  ...
  const updateDeclaration = (id, patch) => {
    setTouched(true);
    setDeclarations(ds => ds.map(d => (d.id === id ? { ...d, ...patch } : d)));
  };
  const removeDeclaration = id => {
    setTouched(true);
    setDeclarations(ds => ds.filter(d => d.id !== id));
  };

  const declErrors = useMemo(
    () => validateDeclarative(schemasData, declarations),
  ...
  const jsonOutput = useMemo(() => {
    ...
    return JSON.stringify(
      generateDeclarationJSON(schemasData, declarations),
      null,
      2,
    );
  }, [schemasData, declarations]);

  const totalErrors = Object.values(declErrors).reduce(
    (n, e) => n + e.length,
    0,
  );
  ...
  const handleDownload = () => {
    ...
    recordMilestone("declarations-generated");
  };
        ...
        <div className="mode-toolbar-inner">
          ...
          <div className="mode-toolbar-right">
            ...
            >
              ...
              {touched && !isValid && !declarations.length && (
        ...
        >
          ...
          <div className="payloads-section">
            <div className="payloads-header">
              <h2>Declarations</h2>
              <ItemPicker
                schemas={schemasData.declarations}
            ...
            {!declarations.length && (
              <div className="empty-state">
                ...
                <p className="empty-hint">
                  Click <strong>＋ Add Declaration</strong> to choose from{" "}
                  {Object.keys(schemasData.declarations).length} declaration
                  types across configurations, activations, and assets.
                </p>
              </div>
            )}
            ...
            {declarations.map(d => {
              ...
              const declType =
                schema?.payload?.declarationtype || d.declarationId;
              const collapsed = collapsedIds.has(d.id);
              const errs = declErrors[d.id] || [];
              return (
                <div
                  key={d.id}
                  className={`payload-form ${showErrors && errs.length ? "has-errors" : ""}`}
                >
                  <div
                    className="payload-form-header"
                    onClick={() => toggleCollapsed(d.id)}
                    style={{ cursor: "pointer" }}
                  >
                    <div>
                      <span className="payload-collapse-arrow">
                        {collapsed ? "▶" : "▼"}
                      </span>
                      <span
                        className={`payload-type-badge badge-decl-${schema?._category || "other"}`}
                      >
                        {declType}
                      </span>
                      {schema?.title && (
                        <span className="payload-title">{schema.title}</span>
                      )}
                    </div>
                    <div className="payload-form-header-right">
                      {showErrors && errs.length > 0 && (
                        <span className="payload-error-count">
                          {errs.length} required field
                          {errs.length !== 1 ? "s" : ""} missing
                        </span>
                      )}
                      <button
                        className="rm-payload-btn"
                        onClick={e => {
                          e.stopPropagation();
                  ...
                  {!collapsed && (
                    <>
                      {schema?.description && (
                        <p className="payload-desc">{schema.description}</p>
                      )}
                      <OsSupportTable
                      ...
                      {showErrors && errs.length > 0 && (
                        <div className="payload-errors">
                          <span className="payload-errors-label">
                            Required:
                          </span>
                          {errs.map(e => (
                          ...
                          <LabelWithHelp
                            ...
                            help={
                              <>
                                ...
                                declaration within your declaration set. Must be
                                stable across updates — the OS uses it to match
                                the declaration to its previously installed
                                version. A UUID is a safe default (pre-filled),
                                but a human-readable reverse-DNS string like{" "}
                                <code>com.acme.wifi.corp</code> works too and is
                                easier to reference from activations.
                              </>
                            }
                          />
                          <input
                            ...
                            value={d.identifier}
                            onChange={e =>
                              updateDeclaration(d.id, {
                                identifier: e.target.value,
                              })
                            }
                            placeholder="Unique identifier (UUID recommended)"
                          />
                        </div>
                        <div className="field">
                          <LabelWithHelp
                            ...
                            help={
                              <>
                                An opaque string the server uses to indicate a
                                specific revision of this declaration. The
                                device compares this to its cached copy — if the
                                tokens differ, it re-applies the declaration.
                                Change this value every time you update the
                                declaration's payload. A new UUID (pre-filled)
                                or an incrementing value like <code>v2</code>{" "}
                                both work.
                              </>
                            }
                          />
                      ...
                      <div className="fields">
                        {(schema?.payloadkeys || []).map(keyDef => {
                          const isMissing =
                            showErrors &&
                            errs.includes(keyDef.title || keyDef.key);
                          return (
                            <div
                              key={keyDef.key}
                              className={`field ${keyDef.presence === "required" ? "required-field" : ""} ${isMissing ? "field-missing" : ""}`}
                            >
                              <FieldLabel
                                title={keyDef.title}
                                keyName={keyDef.key}
                                description={keyDef.content}
                                required={keyDef.presence === "required"}
                                supportedOS={keyDef.supportedOS}
                                payloadSupportedOS={
                                  schema?.payload?.supportedOS
                                }
                                defaultVal={keyDef.default}
                              />
                              <FieldInput
                                keyDef={keyDef}
                                value={d.values[keyDef.key]}
                                onChange={v =>
                                  updateDeclaration(d.id, {
                                    values: { ...d.values, [keyDef.key]: v },
                                  })
                                }
